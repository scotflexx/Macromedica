    ...
    <div className="h-full flex flex-col bg-white">
      ...
      <div className="p-4 border-b border-slate-200/80 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-sm font-bold text-slate-900">{task?.description || 'Bilan Sanguin / Analyse Biologique'}</h2>
              <span className="bg-blue-100 text-blue-700 text-[11px] font-extrabold px-2 py-0.5 rounded-md border border-blue-200">
                Lab Result PDF
              </span>
            </div>
        ...
        <div className="flex items-center gap-1.5 bg-slate-100 p-1 rounded-xl border border-slate-200">
          ...
          <button
            type="button"
            className="px-2.5 py-1 text-xs font-semibold text-slate-700 hover:bg-white rounded-lg transition-all flex items-center gap-1.5 shadow-2xs"
          >
            <Download size={13} />
            PDF
          </button>
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-5">
        ...
        <div className="bg-gradient-to-r from-amber-50 via-orange-50 to-rose-50 border border-amber-200/80 rounded-2xl p-4 shadow-2xs">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-[11px] font-bold bg-rose-100 text-rose-800 px-2 py-0.5 rounded-full border border-rose-300">
          ...
          <div className="flex flex-wrap gap-2">
            {abnormalParams.map((p, idx) => (
              <div
                key={idx}
                className="bg-white/90 border border-rose-200 px-2.5 py-1 rounded-xl flex items-center gap-2"
              >
                <AlertCircle size={13} className="text-rose-600 shrink-0" />
                <span className="text-xs font-bold text-slate-800">{p.param}:</span>
                <span className="text-xs font-extrabold text-rose-700 bg-rose-50 px-1.5 py-0.5 rounded border border-rose-200">
                  {p.value}
                </span>
                <span className="text-[10px] font-bold text-rose-800">({p.label})</span>
              </div>
            ))}
          </div>
        </div>
        ...
        >
          ...
          <div className="border-b border-slate-200 pb-3 mb-4 flex items-center justify-between">
            <div>
              <div className="text-sm font-black text-blue-950 tracking-tight">LABORATOIRE D'ANALYSES MÉDICALES DE CASABLANCA</div>
              <div className="text-[11px] font-semibold text-slate-500 mt-0.5">244 Boulevard Mohamed V, Casablanca • Tel: 0522-889900</div>
            </div>
          ...
          {/* Patient Info Row */}
          <div className="bg-slate-50 rounded-lg p-3 mb-4 border border-slate-200/60 grid grid-cols-3 gap-3 text-xs">
            <div>
              <span className="text-slate-400 font-medium block">Nom & Prénom:</span>
              <strong className="text-slate-900 font-bold">{task?.patientName}</strong>
            </div>
            <div>
          ...
          <table className="w-full text-xs text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-200 text-slate-500 font-semibold bg-slate-50">
                <th className="py-2 px-3">Analyse / Paramètre</th>
                <th className="py-2 px-3">Résultat Obtenu</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {labParameters.map((item, idx) => (
                <tr key={idx} className={item.status === 'high' ? 'bg-rose-50/40' : item.status === 'borderline' ? 'bg-amber-50/30' : ''}>
                  <td className="py-2 px-3 font-semibold text-slate-900">{item.param}</td>
                  <td className={`py-2 px-3 font-extrabold ${item.status === 'high' ? 'text-rose-700' : item.status === 'borderline' ? 'text-amber-700' : 'text-slate-900'}`}>
                    {item.value}
                  </td>
                  <td className="py-2 px-3 text-slate-500">{item.norm}</td>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-4 space-y-2.5">
          ...
          <textarea
            ...
            className="w-full p-3 rounded-xl border border-slate-200 text-xs text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-500 leading-relaxed font-medium bg-slate-50/50"
      ...
      <div className="p-4 border-t border-slate-200 flex items-center justify-between gap-3 shrink-0">
        <button
          ...
          className="px-4 py-2.5 rounded-xl border border-slate-200 text-slate-700 hover:bg-slate-50 text-xs font-bold flex items-center gap-2"
        >
          <Archive size={15} />
          Archiver au dossier
        </button>
        ...
        <button
          ...
          className="px-5 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold shadow-md shadow-blue-500/20 transition-all duration-200 hover:-translate-y-0.5 active:scale-95 flex items-center gap-2 disabled:opacity-50"
        >
